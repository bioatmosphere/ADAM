"""
Apply trained XGBoost model globally using global climate and satellite data.

This script loads the trained XGBoost model and applies it to global gridded data to produce
...
def apply_xgb_model_globally(model: xgb.XGBRegressor, features_df: pd.DataFrame) -> np.ndarray:
    """
    Apply XGBoost model to global features.
...
def main():
    ...
    try:
        print("="*60)
        print("GLOBAL XGBOOST BNPP PREDICTION")
        print("="*60)
        
        # 1. Load trained XGBoost model
        model, model_data = load_trained_xgb_model()
        ...
        # 6. Apply XGBoost model globally
        predictions = apply_xgb_model_globally(model, features_df)
        ...
        print("\nGlobal XGBoost application completed successfully!")
        
    except Exception as e:
        print(f"Error in global XGBoost application: {e}")
        return False
