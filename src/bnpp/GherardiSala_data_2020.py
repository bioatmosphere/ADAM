...
# Instead of reading the CSV file into, directly converting it into a DataFrame
#GS_data_2020 = pd.read_csv('../productivity/globe/supplementary_table_5.csv')
GS_data_2020 = pd.DataFrame(csv_output_data[1:], columns=csv_output_data[0])

# Convert numeric columns to appropriate types
numeric_columns = ['Latitude', 'Longitude', 'MAT', 'MAP']
for col in numeric_columns:
    GS_data_2020[col] = pd.to_numeric(GS_data_2020[col], errors='coerce')  # 'coerce' will convert invalid values to NaN

# Display basic information about the dataset
print("\nDataset Summary:")
print(f"Number of rows: {len(GS_data_2020)}")
print(f"Number of columns: {len(GS_data_2020.columns)}")
print("\nColumn types:")
print(GS_data_2020.dtypes)
...
unique_cover = GS_data_2020['Cover Type'].unique()
...
for cover, color in zip(unique_cover, colors):
    mask = GS_data_2020['Cover Type'] == cover
    ax.scatter(GS_data_2020.loc[mask, 'Longitude'], 
               GS_data_2020.loc[mask, 'Latitude'],
               color=color,
               s=50,
               label=cover,
...
# Adjust legend position and size with enhanced styling
ax.legend(title='Cover Types',
         bbox_to_anchor=(1.05, 1.0),     # Position outside right of plot
...
plt.tight_layout(rect=[0, 0, 0.9, 0.5])

# Set title
ax.set_title(f'Sites ({GS_data_2020.shape[0]}) with BNPP Measurements', fontsize=15)
